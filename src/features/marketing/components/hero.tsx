...
function HeroVisual() {
  return (
    <div className="relative mx-auto w-full max-w-md lg:max-w-none" aria-hidden>
      ...
      <div className="absolute -top-4 end-0 start-8 rotate-[-2deg] rounded-xl border border-border bg-white p-6 shadow-lift sm:start-14">
        <div className="space-y-2.5">
          <div className="mx-auto h-2.5 w-32 rounded-full bg-sand-200" />
          <div className="h-2 w-full rounded-full bg-sand-100" />
          <div className="h-2 w-11/12 rounded-full bg-sand-100" />
          <div className="h-2 w-full rounded-full bg-sand-100" />
          <div className="h-2 w-9/12 rounded-full bg-sand-100" />
          <div className="h-2 w-full rounded-full bg-sand-100" />
          <div className="h-2 w-8/12 rounded-full bg-sand-100" />
        </div>
      </div>

      {/* بطاقة المقابلة في المقدمة */}
      <div className="relative mt-16 rounded-2xl border border-border bg-surface p-6 shadow-lift sm:me-10">
...
export function Hero() {
  return (
    <section className="relative overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 -top-40 h-[32rem] bg-[radial-gradient(ellipse_60%_50%_at_50%_0%,var(--color-brand-100),transparent)] opacity-70"
      />
      ...
      <Container className="relative">
        <div className="grid items-center gap-12 py-16 sm:py-20 lg:grid-cols-2 lg:gap-8 lg:py-28">
          <div className="animate-fade-up text-center lg:text-start">
            <p className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-3.5 py-1.5 text-xs font-medium text-muted-foreground shadow-soft">
              <Sparkles className="size-3.5 text-primary" />
              مدعوم بالذكاء الاصطناعي
            </p>

            <h1 className="mt-6 text-3xl font-bold leading-[1.25] sm:text-4xl lg:text-5xl lg:leading-[1.2]">
              {site.tagline}
            </h1>
